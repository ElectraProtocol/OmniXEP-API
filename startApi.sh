...
kill_child_processes() {
  #kill $SERVER_PID
  uwsgi --stop /tmp/OmniXEP-API.pid
  kill $WEBSOCKET_PID
  rm -f $LOCK_FILE
  exit 1
}
...
LOGDIR=$APPDIR/logs
LOCK_FILE=$APPDIR/OmniXEP-API.lock
...
do
  ...
  if [ ! -f $LOCK_FILE ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        cd $APPDIR/api
        uwsgi -s 127.0.0.1:1088 -p 10 -M --vhost --enable-threads --log-x-forwarded-for --logto $LOGDIR/apps.log --pidfile /tmp/OmniXEP-API.pid --stats /tmp/stats.socket &
    ...
    if [ "$APISHA" != "$CHECKSHA" ]; then
        uwsgi --reload /tmp/OmniXEP-API.pid
        APISHA=$CHECKSHA
        echo Api Reloaded
    fi
